...
class ImagenesRestauranteSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Obtener imágenes reales del public
        $storagePath = public_path('img/restaurantes');
        $imagenes = [];
        ...
        if (is_dir($storagePath)) {
            ...
            foreach ($files as $file) {
                if (in_array(pathinfo($file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                    $imagenes[] = 'img/restaurantes/' . $file;
                }
            }
        }
        
        if (empty($imagenes)) {
            $this->command->info('No hay imágenes en public/img/restaurantes.');
            return;
        }
